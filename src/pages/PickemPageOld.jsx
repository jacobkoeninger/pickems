...
const PickemPage = () => {
  ...
  const isChoiceMade = React.useCallback((choiceId) => {
    return userChoices?.some(uc => uc.pickemChoice.id === choiceId);
  }, [userChoices]);

  const handleChoice = React.useCallback(async (pickemChoiceId) => {
    if (!user?.id) return;
    await createUserPickemChoiceFn({ userId: user.id, pickemChoiceId });
  }, [user?.id, createUserPickemChoiceFn]);

  const handlePickemChoice = React.useCallback(async (pickemChoiceId) => {
    if (!isContestActive(selectedContest)) {
      return;
    }
    await handleChoice(pickemChoiceId);
    if (viewMode === 'single' && currentPickemIndex < filteredPickems?.length - 1) {
      setCurrentPickemIndex(currentPickemIndex + 1);
    }
  }, [handleChoice, selectedContest, viewMode, currentPickemIndex, filteredPickems?.length]);

  // Auto-select user's choices if they own any
  useEffect(() => {
    if (!pickems || !user?.nickname) return;
    
    pickems.forEach(pickem => {
      const userChoice = pickem.choices.find(c => c.nickname === user.nickname);
      if (userChoice && !isChoiceMade(userChoice.id)) {
        handleChoice(userChoice.id);
      }
    });
  }, [pickems, user?.nickname, isChoiceMade, handleChoice]);
  ...
  const renderPickemChoice = (pickem) => {
    ...
    const userChoice = pickem.choices.find(c => c.nickname === user.nickname);
    const userOwnsAChoice = userChoice != null;
        ...
        <div className="space-y-4">
          {pickem.choices.map((choice) => {
            ...
            return (
              <button
                key={choice.id}
                onClick={() => !choiceMade && !userOwnsAChoice && handlePickemChoice(choice.id)}
                ...
                className={`w-full py-4 px-6 border rounded transition-all duration-200
                           ...
                           ${choiceMade 
                             ...
                             : isOwnedChoice
                               ...
                               : userOwnsAChoice
                                 ? 'bg-black border-gray-500 text-gray-500 cursor-not-allowed'
